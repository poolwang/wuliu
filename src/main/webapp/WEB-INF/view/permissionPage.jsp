...
<div class="page-container">
	...
	<!-- bootstrap-table的工具条 -->
	 <span id="toolbar" class="l">
	 	...
	 	<a href="javascript:;" onclick="permission_add()" class="btn btn-primary radius">
	...
	$('#permissionTable').bootstrapTable({
		...
		*/
		...
		columns: [ //表格显示数据对应的表头设置，
			...
			*/
			...
			{field: 'permission.name',title: '父权限'},
		...
		*/
		queryParams: function(params) { 
			...
			//此方法在权限分页或者搜索的时候回自动发送ajax请求调用，并把对应的参数传递给后台
			return {
...
function operationFormatter(value,row,index){
	var html ="<span onclick='permission_edit("+row.id+")'  style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span>&nbsp;&nbsp";
		html+="<span onclick='permission_del("+row.id+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span>";
	return html;			
}
...
function menuFormatter(value,row,index){
	var type = "普通权限";
	if(row.type=='menu'){
		type = "<span style='color:red'>菜单权限</span>";
	}
	return type;
}
...
function permission_add(){
	layer_show("添加权限","permission/edit.do",700,500);
}
/*权限-编辑*/
function permission_edit(permissionId){
	layer_show("修改权限","permission/edit.do?permissionId="+permissionId,700,500);
}
/* 批量删除 */
function datadel() {
	var rows = $("#permissionTable").bootstrapTable('getSelections');
	if (rows.length == 0) {
...
function permission_del(id){
	...
	layer.confirm('确认要删除吗？',{icon: 3, title:'提示'},function(index){
		...
		//使用jquery发送ajax请求
		$.post("permission/delete.do",{permissionId:id},function(resData){
			//关闭询问框
			layer.close(index);
			//弹出一个提示消息
