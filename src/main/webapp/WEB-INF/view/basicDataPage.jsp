...
<div class="page-container">
	...
	<!-- bootstrap-table的工具条 -->
	 ...
	 <shiro:hasPermission name="basicData:delete">
	 	...
	 	<a href="javascript:;" onclick="basicData_add()" class="btn btn-primary radius">
	 	<i class="Hui-iconfont">&#xe600;</i> 添加基础数据</a>
	 	</shiro:hasPermission>
	 </span> 
	
	<!-- 定义一个表格，以供bootstrap-table插件使用 -->
	<table id="basicDataTable" class="table table-border table-bordered table-bg">
	</table>
...
$(function(){
	...
	$('#basicDataTable').bootstrapTable({
		url: 'basicData/list.do',//ajax请求的url地址
		/*
			ajax请求以后回调函数的处理
		...
		*/
		queryParams: function(params) { 
			//此方法在基础数据分页或者搜索的时候回自动发送ajax请求调用，并把对应的参数传递给后台
			return {
...
function operationFormatter(value,row,index){
	var html ="<shiro:hasPermission name='basicData:update'><span onclick='basicData_edit("+row.baseId+")'  style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span></shiro:hasPermission>&nbsp;&nbsp";
	html+="<shiro:hasPermission name='basicData:delete'><span onclick='basicData_del("+row.baseId+")'  style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span></shiro:hasPermission>";
	return html;			
}
...
*/
/*基础数据-增加*/
function basicData_add(){
	layer_show("添加基础数据","basicData/edit.do",700,500);
}
/*基础数据-编辑*/
function basicData_edit(baseId){
	layer_show("修改基础数据","basicData/edit.do?baseId="+baseId,700,500);
}
/* 批量删除 */
function datadel() {
	var rows = $("#basicDataTable").bootstrapTable('getSelections');
	if (rows.length == 0) {
...
/*基础数据-删除*/
function basicData_del(id){
	layer.confirm('确认要删除吗？',{icon: 3, title:'提示'},function(index){
		//使用jquery发送ajax请求
		 $.post("basicData/delete.do",{baseId:id},function(resData){
			//关闭询问框
				layer.close(index);
				//弹出一个提示消息
