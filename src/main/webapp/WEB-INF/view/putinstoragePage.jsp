...
<body>
	<article class="page-container">
		<form class="form form-horizontal" id="putinstorageForm">
			<!-- 隐藏域 ，订单状态-->
			...
			<div class="row cl">
				<div class="col-xs-4 col-sm-4">
					<label class="form-label col-xs-4 col-sm-4">体积费率：</label> <input
						id=volumnUnitPrice name="volumnUnitPrice" placeholder="实际体积费率">
				</div>
				<div class="col-xs-4 col-sm-4">
					<label class="form-label col-xs-4 col-sm-4">重量费率：</label> <input
						id=weightUnitPrice name="weightUnitPrice" placeholder="实际重量费率">
				</div>
				<div class="col-xs-4 col-sm-4">
					<label class="form-label col-xs-4 col-sm-4">入库选择：</label> <select
						...
						<c:forEach items="${wareHouses }" var="w">
							<option value="${w.baseId }">${w.baseName }</option>
						</c:forEach>
					</select>
				...
				<table border="1">
					...
					<c:forEach items="${orderDetails }" var="o">
						...
						<tr height="50">
							...
							<td id="cargoLong"><input name="orderDetails[][cargoLong]" placeholder="货物长" onkeyup="totalValumn(this)"></td>
							<td id="cargoWeight"><input
								name="orderDetails[][cargoWeight]" placeholder="货物宽" onkeyup="totalValumn(this)"></td>
							<td id="cargoHeight"><input
								name="orderDetails[][cargoHeight]" placeholder="货物高" onkeyup="totalValumn(this)"></td>
							<td id="valumn"><input name="orderDetails[][valumn]" placeholder="货物真实体积"></td>
							<!-- 只有空运时，才需要核算重量，海运和陆运默认重量为0，不需要核算 -->
							<td id="weight"><c:if
									test="${freightMethod.baseName eq '空运'}">
							<input name="orderDetails[][weight]" placeholder="货物实际重量">
							</c:if> 
							<c:if test="${freightMethod.baseName ne '空运'}">
									0
							</c:if></td>
						</tr>
					</c:forEach>
...
<script type="text/javascript" src="lib/bootstrap_table/bootstrap-table.js"></script>
...
<script type="text/javascript" src="lib/zTree_v3/js/jquery.ztree.all.min.js"></script>
	...
	<script type="text/javascript"
		...
		<script type="text/javascript">
		function totalValumn(obj){
		var pa = $(obj).parent().parent();
		var inputs = pa.find("input");
		var totals = inputs.eq(0).val()*inputs.eq(1).val()*inputs.eq(2).val()/1000;
		//console.log(totals);
		inputs.eq(3).val(totals);
		}
		
		</script>
	<script type="text/javascript">
		$(function() {
			$("#putinstorageForm").validate({
